...
```{r fig.width=3, fig.height=1, out.width='70%'}
ggplot(data = NLSY, aes(x=Edu2006)) + 
  geom_boxplot() + 
  facet_wrap(~Gender) + 
  xlab('Years of Education')
```
```{r}
summary(NLSY$Edu2006)
print(paste0('Standard Deviation: ', round(sd(NLSY$Edu2006),3)))
```
...
```{r fig.width=2.5, fig.height=1.5, out.width='70%'}
...
ggplot(data = NLSY, aes(x=log(Income2005))) + 
  geom_boxplot() + 
  facet_grid(Gender~Edu2006,scale='free_y') + 
  xlab('Income')
```
